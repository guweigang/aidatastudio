...
- `VTABLE_WINDOWS_SIDECAR_V_FLAGS`: Windows sidecar V compile flags. Defaults
  to `-cc msvc -show-c-output -d link_quickjs`.
- `TAURI_UPDATER_PUBLIC_KEY`: public key generated by
  `cargo tauri signer generate`; required when `publish_github_release` is
  enabled.
- `TAURI_UPDATER_ENDPOINT`: optional updater manifest URL. Defaults to
  `https://github.com/guweigang/aidatastudio/releases/latest/download/latest.json`.
...
- `TAURI_SIGNING_PRIVATE_KEY`: private key generated by
  `cargo tauri signer generate`; required when publishing updater-enabled
  releases.
- `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`: optional password for the updater
  signing private key.
